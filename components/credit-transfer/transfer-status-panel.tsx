...
export function TransferStatusPanel({ state, title, body, icon, children }: TransferStatusPanelProps) {
  ...
  return (
    <section className={`rounded-xl border p-5 sm:p-6 ${tone.panelBg} ${tone.panelBorder}`}>
      <div className="flex items-start gap-4">
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-[var(--background)] ${tone.icon}`}>
          <Icon aria-hidden="true" className="h-5 w-5" />
        </div>
        <div className="min-w-0 flex-1">
          <span className={`inline-flex h-6 items-center rounded-full px-2.5 text-xs font-semibold ${tone.badgeBg} ${tone.badgeText}`}>
            {info.label}
          </span>
